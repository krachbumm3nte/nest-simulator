...
class Network:
    ...
    def __init__(self, dims, noise=True, noise_std=0.1, init_self_pred=False) -> None:
        self.dims = dims
        self.noise = noise
    ...
    def setup_populations(self, self_predicting):
        ...
        for lr in range(1, self.L):
            ...
            print(syn_spec_ff_pp['weight'].shape)
        ...
        self.mm_pyr_l = nest.Create('multimeter', 1, {'record_from': ["V_m.a_lat", "V_m.a_td", "V_m.b", "V_m.s"]})
        self.mm_intn = nest.Create('multimeter', 1, {'record_from': ["V_m.s"]})
        self.mm_pyr_m = nest.Create('multimeter', 1, {'record_from': ["V_m.s"]})
        nest.Connect(self.mm_pyr_l, self.pyr_pops[1])
        nest.Connect(self.mm_pyr_m, self.pyr_pops[2])
