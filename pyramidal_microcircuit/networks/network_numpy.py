...
class NumpyNetwork(Network):
    ...
    def simulate(self, train_function, enable_recording=True, plasticity=True):
        ...
        if self.le:
            ...
            self.layers[-1].update(self.phi(self.layers[-2].u_pyr["forw"]), train_function(), plasticity, noise_on=noise_on)
        else:
            ...
            for n in range(1, len(self.layers) - 1):
                self.layers[n].update(self.phi(self.layers[n - 1].u_pyr["soma"]), self.layers[n + 1].u_pyr["soma"],
                                     plasticity, noise_on=noise_on)
            self.layers[-1].update(self.phi(self.layers[-2].u_pyr["soma"]), train_function(), plasticity, noise_on=noise_on)
        
        for l in self.layers:
            l.apply(plasticity)
        
        if enable_recording:
            if self.iteration*self.dt - self.record_interval < 1e-3:
                self.record_state()
            self.iteration += 1
    ...
    def record_state(self):
        ...
        for i, weight_dict in enumerate(self.copy_weights()):
            for key, weights in weight_dict.items():
                self.weight_record[i][key] = np.concatenate((self.weight_record[i][key], np.expand_dims(weights, axis=0)), axis=0)
