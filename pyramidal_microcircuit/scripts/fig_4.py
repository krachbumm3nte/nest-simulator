...
if __name__ == "__main__":
    ...
    for config in all_configs:
        ...
        t_success = np.where(acc[:, 1] == 1)[0]

        print(t_success[0], acc[t_success[0], [0]])
        if len(t_success) == 0:
            t_success = 10000
        else:
            t_success = acc[t_success[0], 0]
        ...
        if n_hidden in [10, 30, 100]:
            ...
            ax0.plot(times, utils.rolling_avg(acc, filter_window), label=r"$n_{{hidden}}={}$".format(n_hidden), color="orange", linestyle=linestyles[n_hidden])
