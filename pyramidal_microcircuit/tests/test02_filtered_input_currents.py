...
compartments = nest.GetDefaults(pyr_model_rate)["receptor_types"]

print(compartments)
step_generator = nest.Create("step_current_generator")
print(compartments["soma_curr"])
pyr_in_2 = nest.Create(pyr_model_rate, 1, input_params)
...
step_generator.set(amplitude_values=np.array(stim_amps)*tau_input,
                   amplitude_times=np.cumsum(sim_times).astype(float) - 50 + delta_t)
...
plt.plot(mm_in.get("events")["times"]/delta_t, mm_in.get("events")
         ["V_m.s"], label="pyramidal neuron with injected current")
plt.plot(mm_in_2.get("events")["times"]/delta_t, mm_in_2.get("events")
         ["V_m.s"], label="pyramidal neuron with dc generator")
plt.legend()
plt.show()
