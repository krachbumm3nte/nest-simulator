...
class MathematicaNetwork:
    ...
    def setup_populations(self, self_predicting):
        ...
        self.y = np.asmatrix(np.random.random(self.dims[2])) # add miniscule offset 

        self.conn_names = ["hx", "yh", "ih", "hi", "hy"]

        conn_setup = {
            "hx": {"eta":eta_hx * 2.05, "in":self.dims[0], "out":self.dims[1]},
            "yh": {"eta":eta_yh * 2.05, "in":self.dims[1], "out":self.dims[2]},
            "ih": {"eta":eta_ih * 2.05, "in":self.dims[1], "out":self.dims[2]},
    ...
    def train(self, T):
        ...
        for i in range(int(T/delta_t)):
            if (self.y == 0).any():
                print(self.y)
            y = phi_inverse(self.y)
            self.simulate(y)
        self.output_pred =  phi( (g_d / (g_d + g_l)) + self.conns["yh"]["w"] * phi((g_d / (g_d + g_l + g_a)) * self.V_bh).T).T

        self.output_loss.append(mse(np.asarray(self.output_pred), np.asarray(self.y)))

    def test(self):
        y = np.asmatrix(np.zeros(self.dims[2]))
        pass
    ...
    def simulate(self, y):
        ...
        delta_u_y = -(g_l + g_d + g_a) * self.U_y + g_d * self.V_by + g_s * y
