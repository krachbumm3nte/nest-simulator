...
class MathematicaNetwork:
    ...
    def setup_populations(self, self_predicting):
        ...
        self.conns["hi"]["eta"] = eta_int_pyr
        self.conns["ih"]["eta"] = eta_pyr_int
    ...
    def simulate(self, T):
        for i in range(int(T/delta_t)):
            ...
            self.conns["yh"]["dt_w"] = -self.conns["yh"]["t_w"] + np.outer(self.r_y - phi((g_d * self.V_by)/(g_l + g_d)), self.r_h)
            self.conns["hy"]["dt_w"] = -self.conns["hy"]["t_w"] + np.outer(self.r_h - phi(self.conns["hy"]["w"] * self.r_y.T).T, self.r_y)
            self.conns["ih"]["dt_w"] = -self.conns["ih"]["t_w"] + np.outer(self.r_i - phi((g_d * self.V_bi)/(g_l + g_d)), self.r_h)
