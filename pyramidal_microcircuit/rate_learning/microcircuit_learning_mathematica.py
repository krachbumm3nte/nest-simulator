...
for run in range(n_runs):
    ...
    if run % 100 == 0:
        ...
        v_intn = pd.DataFrame.from_dict(net.mm_i.events).sort_values("times")
        v_intn = v_intn.groupby("senders")

        v_pyr = pd.DataFrame.from_dict(net.mm_y.events).sort_values("times")
        ...
        # plot apical voltage
        events = pd.DataFrame.from_dict(net.mm_h.events).sort_values("times")
