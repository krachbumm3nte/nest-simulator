...
nest::pp_cond_exp_mc_pyr_dynamics( double, const double y[], double f[], void* pnode )
{
  ...
  for ( size_t n = 1; n < N::NCOMP; ++n )
  {
    ...
    f[ S::idx( n, S::V_M ) ] = -node.P_.pyr_params.g_L[ n ] * V_dnd + I_dend + I_conn_s_d / node.P_.pyr_params.C_m;
  ...
  f[ S::idx( N::SOMA, S::V_M ) ] =
    ( -I_L + I_conn_d_s + node.B_.I_stim_[ N::SOMA ] + node.P_.I_e[ N::SOMA ] ) / node.P_.pyr_params.C_m;
